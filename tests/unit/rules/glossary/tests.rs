...
fn banned_term_upgrades_existing_same_span_issue() {
    ...
    // plus the internal glossary-banned marker so TM cannot downgrade the only
    // report.
    let glossary = ProjectGlossary {
        banned: vec!["線程".into()],
        ..ProjectGlossary::default()
...
fn banned_term_upgrades_larger_covering_issue() {
    // Banned 用戶 inside an existing 用戶介面 issue: the compound issue is the
    // user-visible alert, but it must carry glossary-banned provenance so TM
